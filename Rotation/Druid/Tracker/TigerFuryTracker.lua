...
function TigerFuryTracker.new()
    ...
    self.tigerFuryIsIp = Helpers:HasBuff("player", "Ability_Mount_JungleTiger")
    return self
end
...
function TigerFuryTracker:onEvent(event, arg1)
    if event == "CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS" and string.find(arg1, ABILITY_TIGER_FURY) then
        Logging:Debug(ABILITY_TIGER_FURY.." is up")
        self.tigerFuryIsIp = true
    elseif event == "CHAT_MSG_SPELL_AURA_GONE_SELF" and string.find(arg1, ABILITY_TIGER_FURY) then
